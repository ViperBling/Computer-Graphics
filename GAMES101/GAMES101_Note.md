...
解决问题的一个思路就是在采样之前先进行一次滤波（模糊化处理）。先模糊后采样。
...
### 8. Antialiasing By Supersampling(SSAA)
...
屏幕中多个物体显示顺序的问题，可以像画家画画那样，从后向前依次显示：

![image-20200721185756396](GAMES101_Note.assets/image-20200721185756396.png)

但是这个方法解决不了交替重叠的问题：
![image-20200721185836342](GAMES101_Note.assets/image-20200721185836342.png)

为了解决这个问题，可以采用Z-Buffer算法。

### Z-Buffer

![image-20200721190416978](GAMES101_Note.assets/image-20200721190416978.png)

Z-Buffer对每个像素进行排序，在像素内记录深度信息。两个缓存分别存储颜色信息和深度信息。如下图：

![image-20200721190707850](GAMES101_Note.assets/image-20200721190707850.png)

如上图，首先绘制地板的时候存储了地板像素的深度值，然后绘制物体的时候物体也有深度值，要比地板的深度值小（因为物体离我们近），所以要对被覆盖的像素深度值进行更新。

![image-20200721193421692](GAMES101_Note.assets/image-20200721193421692.png)

![image-20200722161751142](GAMES101_Note.assets/image-20200722161751142.png)

算法过程如上图，初始状态无限远，然后绘制深度为5的三角形，更新深度，然后又进来一个新的三角形，按照深度值绘制图形。Z-Buffer算法得时间复杂度是$O(n)$，$n$是三角形个数。同时Z-Buffer和三角形绘制得顺序是没有关系的，因为我们最终得到的是所有三角形中深度最小的那个。

## Shading

只有上面的步骤在显示图像的时候也会造成失真，因为我们没有着色（也就是随着光照变化产生的变化）。在本课程中，Shading的意思就是对不同物体应用不同的材质。

### Blinn-Phong Reflection Model

![image-20200722165302405](GAMES101_Note.assets/image-20200722165302405.png)

在上图中，茶杯上面会有高光，同样的茶杯在不同方向的光线作用会有漫反射现象，背对光源的物体会受到间接光照（环境光照）。在某个点（Shading Point），有法线、观测方向、光线向量：

![image-20200722165729628](GAMES101_Note.assets/image-20200722165729628.png)

要注意的是，我们考虑着色时只考虑局部，也就是当前的着色点，而不考虑其他物体的存在。

![image-20200722170110159](GAMES101_Note.assets/image-20200722170110159.png)

### Diffuse Reflection

漫反射中，光线会被均匀地反射到各个方向。那么怎么确定物体接受光线的多少呢（也就是明暗度），如果把光看成离散的，那么光线方向和物体表面法线的夹角可以用来确定亮度，也就是光线的通量：

![image-20200722170412924](GAMES101_Note.assets/image-20200722170412924.png)



就类似四季的形成是因为太阳光线和地球表面的夹角一样（夏季太阳直射，光线充足；冬季太阳斜射，光线较少），这里也可以用夹角表示物体表面明暗变化。

对于光源来说，如果是点光源的话，它的能量（光线强度）和距离平方成反比：

![image-20200722171139872](GAMES101_Note.assets/image-20200722171139872.png)

如上，单位距离下光强度为$I$，那么距离为$r$的光强度是$I/r_2$。根据能量守恒定律，在不同的距离的光辐射的能量是一样的（假设没有光损失），也就是在单位距离下辐射出的球面上能量是$I$，那么在其他距离的球面上的总能量也是$I$，只不过其他距离的球面上能量密度发生了变化。因为总能量一定，面积越大，密度越小，所以就和$r^2$成反比。

那么当我们知道了一个点光源，以及物体和点光源的距离，就知道有多少光线传播到了物体附近。我们就知道了如何得到明暗强度：

![image-20200722171951831](GAMES101_Note.assets/image-20200722171951831.png)

$k_d$是物体本身的反射率，等于1时，所有的光都被反射；等于0时，所有的光都被吸收，如果将其表示成RGB的向量，就可以在着色点上定义颜色了。$I/r^2$是点光源抵达物体的能量。$\max(0, \boldsymbol{n\cdot l})$表示物体实际反射的光线。

![image-20200722172353524](GAMES101_Note.assets/image-20200722172353524.png)

同时，由漫反射的特性和上面的公式可以看出，不论我们视点的方向是什么，颜色都不会变，这是符和常理的。当我们看一些不光滑的表面，例如陶器、纸张、木头的表面时，其明暗程度和颜色并不会随着我们观察位置的变化而变化。

# Lecture08. Shading2（Shading Pipeline and Texture Mapping）

- Blinn-Phong Reflectance Model: Specular and ambient terms
- Shading frequencies
- Graphicss pipeline
